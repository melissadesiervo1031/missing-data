...
### Bayes estimated missing data-simulated missing days-high sdp (0.1) and phi (0.8)
``` {r , echo=FALSE, fig.cap=" ", fig.show="hold", out.width="50%"}
fit.miss<- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/full_sim_day_bayes_sdp_1_phi_8_b0_1_b1_1.RDS")
...
ggplot(data=y_combined, aes(x=date, y=estimated))+
  ...
  theme(legend.position="top")+
  ylab("Imputed data (95% CI)")+
  xlab("Time")+
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())

##Get observed (non-missing) data
obs_data_loc<-which(!is.na(y_miss[[2]]))
obs_data_ts<-y_miss[[2]][obs_data_loc]
obs_data_time<-date[obs_data_loc]
obs<-as.data.frame(cbind(obs_data_time,obs_data_ts))
##Plot t-s of missing observed data
ggplot(data=y_combined, aes(x=date, y=estimated))+
  geom_point(data=obs, aes(x=obs_data_time, y=obs_data_ts, color="gray"), size=3)+
  geom_errorbar(data=y_combined,aes(x=date,ymin=low, ymax=high))+ 
  geom_point(aes(color="red"), size=3)+
  theme_classic()+
  theme(axis.title.x=element_text(size=18,colour = "black"))+
...
### Bayes parameter-simulated missing days-high sdp (0.1) and phi (0.8)
``` {r , echo=FALSE, fig.cap=" ", fig.show="hold", out.width="50%"}
##Load model output
...
\
\
### Amelia parameter-simulated missing days sdp=0.1, phi=0.8
``` {r , echo=FALSE, fig.cap="", fig.show="hold", out.width="50%"}
...
##Fix known values 
fit_sim_summary_amelia$known.param<-c(rep(c(0.1, 0.8, 0.1, 0.1), times=1),rep(rep(c(0.1, 0.8, 0.1, 0.1), times=5), times=7))
fit_sim_summary_amelia$imput<-as.factor(c(rep(0, times=4),rep(rep(1:5, each=4), times=7)))

##Calculate denominator of rwci for easier calculation
fit_sim_summary_amelia$rwci.den<-c(rep(fit_sim_summary_amelia$high[1:4]-fit_sim_summary_amelia$min[1:4], times=1),rep(rep(fit_sim_summary_amelia$high[1:4]-fit_sim_summary_amelia$min[1:4], times=5), times=7))
...
diff<-ddply(fit_sim_summary_amelia, c("prop.missing", "param", "imput"),summarize,
             bias=(avg-known.param), 
             rwci= (ci.max-ci.min)/rwci.den
             )
fit_sim_summary_amelia$bias<-fit_sim_summary_amelia$avg-fit_sim_summary_amelia$known.param
fit_sim_summary_amelia$bias<-diff$bias
fit_sim_summary_amelia$rwci<-diff$rwci
...
fit_summary<-left_join(fit_sim_summary_amelia, summ, by=c("param", "prop.missing"))
...
### Simulated-randomly missing days-lower error 
$$
GPP=\beta_0+\phi\times GPP_{t-1}+\beta_1\times X_{light}+\epsilon_{sdp}
$$
$\beta_0$=0.1\
...
### Bayes estimated missing data-simulated missing days-sdp=0.01, phi= 0.8
``` {r , echo=FALSE, fig.cap=" ", fig.show="hold", out.width="50%"}
fit.miss<- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/full_sim_day_bayes_sdp_1_phi_8_b0_1_b1_1.RDS")
...
\
\
### Amelia parameter-simulated missing days sdp=0.01, phi=0.2
``` {r , echo=FALSE, fig.cap="", fig.show="hold", out.width="50%"}
fit_sim_summary_amelia <- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/summary_sim_day_amelia_sdp_01_phi_2_b0_1_b1_4.RDS")
known.data.sim<- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/known_sim_day_amelia_sdp_01_phi_2_b0_1_b1_4.RDS")



ggplot(data=fit_sim_summary_amelia, aes(x=avg, y=prop.missing ))+
    geom_point(aes( color=param, group=param),size=3,position=position_dodge(0.5))+
    theme_classic()+
    geom_errorbar(aes(xmin=ci.min, xmax=ci.max,color=param, group=param), width=0.2,position=position_dodge(0.5))+
    theme(legend.position="top")+
    ...
    xlab("Parameter Estimate")+
    scale_color_manual(values=c("blue", "green", "darkgray", "black"))+
    scale_x_continuous(limits=c(-.3,0.9))+
    theme(axis.title.x=element_text(size=18,colour = "black"))+
    theme(axis.title.y=element_text(size=18,colour = "black"))+
    theme(axis.text.y=element_text(size=18,colour = "black"))+
    theme(axis.text.x=element_text(size=18,colour = "black"))+
...
### Amelia parameter-simulated missing days sdp=0.01, phi=0.8
``` {r , echo=FALSE, fig.cap="", fig.show="hold", out.width="50%"}
...
fit_summary_bayes <- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/summary_lowmiss2_day_bayes_light_Q.RDS")
known.data<- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/known_lowmiss2_day_bayes_light_Q.RDS")
...
ggplot(data=fit_summary_bayes, aes(x=mean, y=prop.missing ))+
  ...
  scale_x_continuous(limits=c(-3,1.5), breaks= c(-3,-2,-1,0,1,1))+
  theme(axis.title.x=element_text(size=18,colour = "black"))+
  theme(axis.title.y=element_text(size=18,colour = "black"))+
  theme(axis.text.y=element_text(size=18,colour = "black"))+
  theme(axis.text.x=element_text(size=18,colour = "black"))+
  geom_vline(xintercept = known.data$known.data,color=c("darkgray", "black", "green", "blue", "red"))
...
#setwd("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions")
#fit.miss<- readRDS("full_bayes_lowmiss2.RDS")
fit_summary_bayes <- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/summary_amelia_lowmiss2_model_var.RDS")
known.data<- readRDS("C:/Users/matt/IDrive-Sync/Postdoc/Estimating missing data/daily_predictions/known_data_amelia_lowmiss2_model_var.RDS")
...
##Plot parameters
ggplot(data=fit_summary_bayes, aes(x=avg, y=prop.missing ))+
  geom_point(aes( color=param, group=param),size=3,position=position_dodge(0.5))+
  theme_classic()+
  geom_errorbar(aes(xmin=ci.min, xmax=ci.max,color=param, group=param), width=0.2, size=0.5,position=position_dodge(0.5))+
  theme(legend.position="top")+
  ylab("Percent of Missing Data")+
  xlab("Parameter Estimate")+
  scale_color_manual(values=c("blue", "green",  "red","darkgray", "black"))+
  ...
  theme(axis.text.y=element_text(size=18,colour = "black"))+
  theme(axis.text.x=element_text(size=18,colour = "black"))+
  geom_vline(xintercept = known.data$mean,color=c("darkgray", "black", "green", "blue", "red"))
