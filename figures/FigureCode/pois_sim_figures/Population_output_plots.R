...
#               "r_se" = x$se["r"],
#               "alpha_se" = x$se["alpha"],
#               "r_CI_lower" = x$lower[1],
#               "r_CI_upper" = x$upper[1],
#               "alpha_CI_lower" = x$lower[2],
#               "alpha_CI_upper" = x$upper[2],
#               "status" = "good")
#           }
#         ),
...
paramUpperCILong <- ricDat_new %>% 
  pivot_longer(cols = c(r_CI_upper, alpha_CI_upper), 
               values_to = "CI_upper", 
               names_to = "param", 
               names_transform = function(x) str_split(string = x, pattern = "_", simplify = TRUE)[,1]) %>% 
...
paramLowerCILong <- ricDat_new %>% 
  pivot_longer(cols = c(r_CI_lower, alpha_CI_lower), 
               values_to = "CI_lower", 
               names_to = "param", 
               names_transform = function(x) str_split(string = x, pattern = "_", simplify = TRUE)[,1]) %>% 
  #mutate(param = str_replace(param, "_se", "")) %>% 
...
figDat_cov_temp <- ricDat_new_long %>% 
  mutate(CI_lower = CI_lower,#paramEst - 1.96*paramSE,
         CI_upper = CI_upper#paramEst + 1.96*paramSE
         ) %>% 
  filter(param != "sigma") %>% 
  filter(!is.na(CI_lower))# randomly there are some model runs that don't have SE? 

# is the true parameter within the 95% CI? 
figDat_cov_temp$coverage <- c(figDat_cov_temp$paramSim >= figDat_cov_temp$CI_lower & 
                                figDat_cov_temp$paramSim <= figDat_cov_temp$CI_upper)
...
poiss_paramRecovery_SE_MAR2<-poiss_paramRecovery_SE_MAR+ theme(axis.title.x=element_blank(), axis.text.x=element_blank(), legend.text = element_text(size=7))+guides(color = 'none')#guide_legend(nrow = 2))

poiss_paramRecovery_coverage_MAR2<-poiss_paramRecovery_coverage_MAR+theme(legend.text = element_text(size=7))+guides(color = 'none') #guide_legend(nrow = 2))
...
poiss_paramRecovMAR <- poiss_paramRecovery_bias_MAR2 / poiss_paramRecovery_SE_MAR2 / poiss_paramRecovery_coverage_MAR2 + plot_annotation(tag_levels = c('A'))
