...
(gauss_paramRecovery_bias <- ggplot(data = figDat_lines2, aes(x = amtMiss, y = paramDiff_med)) +
   ggh4x::facet_grid2(~factor(param, levels = c( "Intercept","Phi", "Beta covariates")) 
   ...
   xlim(c(-0.03,0.55)) + 
   #ylim(c(0,.3)) +
   scale_colour_brewer(palette = "Dark2",labels = c("Data Deletion-Simple", "Data Deletion-Complete","Multiple Imputations","Kalman Filter", "Data Augmentation" ))
)
