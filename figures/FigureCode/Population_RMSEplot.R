...
forecasts_avg <- forecasts_long %>% 
  #filter(Year > 2013) %>% 
  group_by(Year, missingness, autocorr_binned, propMiss_binned) %>% 
...
(forecastFig_p <- ggplot() + 
    ...
    geom_line(data = forecasts_avg, aes(x = Year, y = Estimate_mean, col = propMiss_binned, group = propMiss_binned), alpha = .8) +
    theme_classic() +
