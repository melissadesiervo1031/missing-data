...
(rmse_NoLineErrorBar <- ggplot(data = RMSE_errorBar) +
    ...
    theme(
      ...
      plot.title = element_text(vjust = -26, hjust = .12),
...
rmse_NoLineErrorBar_sim <- rmse_NoLineErrorBar_sim + 
  labs(title = "Forecast RMSE for simulated time series") +  guides(shape = guide_legend(title = "No. of \nSimulations")) 
