### this R script simulates a Ricker model w/ Poisson error ##
...
simRicker_demoerror <- function(r, alpha, data,time) {
...
simRicker_nbinomerror <- function(r, alpha, data,time, theta) {
...
results[1,1] <- 300  ## start w/ first time step at K###
...
for (i in 1:length(missingProbs)) {
  ...
  simmissingdf_i <- as.data.frame(lapply(simpoptry1,
                                         function(cc) cc[sample(c(TRUE, NA),
                                                                ...
                                                                size = length(cc),
                                                                replace = TRUE) ]))
  simmissingdf_i$popplus1 <- c( simmissingdf_i$Broods[-1], NA)
