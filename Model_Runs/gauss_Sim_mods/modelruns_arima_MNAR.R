...
for (i in 1:length(gauss_sim_MNAR)) {
  ...
  gauss_sim_CurSim_vals <- gauss_sim_MNAR[[CurSim]]$y
  #gauss_sim_CurSim_vals <- gauss_sim_MNAR$gauss_sim997_randMiss_autoCorr_50$y
  gauss_sim_CurSim_params <- gauss_sim_MNAR[[CurSim]]$sim_params
  #gauss_sim_CurSim_params <- gauss_sim_MNAR$gauss_sim997_randMiss_autoCorr_50$sim_params
  ...
  arima_drop_MNAR<- fit_arima_dropmissing(sim_list = gauss_sim_CurSim_vals[2:16],
  ...
  arimadrop_MNAR_df$curSim <- CurSim
  ...
  arimadrop_MNAR_preds$curSim <- CurSim
  ...
  arima_drop_CC_MNAR <- fit_arima_dropmissing_CC(sim_list = gauss_sim_CurSim_vals[2:16],
                                                sim_pars = gauss_sim_CurSim_params, 
                                                forecast = TRUE, 
                                                forecast_days = 73,
                                                dat_full = dat_full)
  ...
  ########### formatting for figure #############
  # save arima model parameters
  arimadropCC_MNAR_df <- arima_drop_CC_MNAR$arima_pars
  arimadropCC_MNAR_df$missingness <- 'MNAR'
  arimadropCC_MNAR_df$type <- 'dropNA_complete'
  arimadropCC_MNAR_df$curSim <- CurSim
  
  # save arima forecasts
  arimadropCC_MNAR_preds <- arima_drop_CC_MNAR$arima_forecast
  arimadropCC_MNAR_preds$missingness <- 'MNAR'
  arimadropCC_MNAR_preds$type <- 'dropNA_complete'
  arimadropCC_MNAR_preds$curSim <- CurSim
  
  #####################################################
  #### MODEL RUN KALMAN FILTER ##############
  #########################################################
  arima_kalman_MNAR<- fit_arima_Kalman(sim_list = gauss_sim_CurSim_vals[2:16],
                                      sim_pars = gauss_sim_CurSim_params, 
                                      forecast = TRUE, 
                                      forecast_days = 73,
                                      dat_full = dat_full)
  ...
  arimaKalman_MNAR_df$curSim <- CurSim
  ...
  arimaKalman_MNAR_preds$curSim <- CurSim
  #######################################################################################################
  ...
  arima_mi_MNAR <-  fit_arima_MI(sim_list = gauss_sim_CurSim_vals[2:16],
                                sim_pars = gauss_sim_CurSim_params, 
                                forecast = TRUE, 
                                forecast_days = 73,
                                imputationsnum=5,
  ...
  arimaMI_MNAR_preds$curSim <- CurSim
  #############################################################################################################
  ...
  preds_MNAR_all <- rbind(
    arimadrop_MNAR_preds, 
    arimadropCC_MNAR_preds,
    arimaKalman_MNAR_preds, 
    arimaMI_MNAR_preds
  )
  
  # add in the "simulation number" for this iteration (which is stored in the name of the data list element)
 CurSim
  #   str_sub(string = names(gauss_sim_MNAR[[CurSim]]), 
  #                    start = str_locate_all(
  #                      pattern = "_", string  = names(gauss_sim_MNAR[CurSim])
  #                    )[[1]][1,1] + 4,
  #                    end = str_locate_all(
  #                      pattern = "_", string  = names(gauss_sim_MNAR[CurSim])
  #                    )[[1]][2,1]-1
  ...
  params_MNAR_all$sim_no <- CurSim
  preds_MNAR_all$sim_no <- CurSim
  
  # Write the output to the folder which will contain all output files as separate csv
  #    files with a single line of data.
  write.csv(params_MNAR_all, file = paste0(OutFile, CurSim,"_params.csv"), row.names = FALSE)
  write.csv(preds_MNAR_all, file = paste0(OutFile, CurSim,"_predValues.csv"), row.names = FALSE)
