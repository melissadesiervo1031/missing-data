...
fit_arima_MI <- function(sim_list, sim_pars, imputationsnum){
  ...
  #                                                m=imputationsnum,  
  #                                                lags = "GPP")) ## lags by 1 day ##
  # amelia1sim <-lapply(X = simmissingdf  ,
  #                     FUN = function(X)   amelia(X, ts="days",
  #                                                m=imputationsnum,
  #                                                polytime = 2)) ## lags by 1 day ##
  amelia1sim <-lapply(X = simmissingdf  ,
                      FUN = function(X)   amelia(X, ts="days",
                                                 m=imputationsnum, lags = "GPP",
                                                 polytime = 2)) ## lags by 1 day ##
  ...
  amelias11sim<-map(amelia1sim , ~.[["imputations"]])
  # amelias11sim3<-map(amelia1sim3 , ~.[["imputations"]])
...
arima_mi_MAR <-  fit_arima_MI(gauss_sim_randMiss_autoCorr_01[[1]]$y,
                              gauss_sim_randMiss_autoCorr_01[[1]]$sim_params,
                              imputationsnum=5)
