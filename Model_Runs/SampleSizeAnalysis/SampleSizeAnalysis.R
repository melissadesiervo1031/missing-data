...
for (i in 1:length(sim_list_miss_short)) {
  CurSim <- i
  ...
  params_MAR_all$simParams <- c(gauss_sim_CurSim_params$phi, gauss_sim_CurSim_params$beta[1], 
...
(gauss_paramRecovery_bias <- ggplot(data = mod_df_bin %>% filter(parameters != "sigma"), aes(x = #as.factor
  ...
  #ylim(c(-1, 1)) #+
   #xlim(c(-0.03,0.65))# + 
   #ylim(c(0,.3)) +
   scale_color_discrete(type = c( "#D55E00", "#0072B2", #"#CC79A7", 
                                  "#009E73", "#E69F00"),
    labels = c( "Data Deletion-Complete", "Kalman Filter","Multiple Imputation",#"Data Augmentation",  
...
(sampleSizeAnalysisFig <- ggarrange(gauss_paramRecovery_bias2, gauss_paramRecovery_SE2, 
