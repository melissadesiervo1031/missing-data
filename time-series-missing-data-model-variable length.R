...
fit_summary_pars_bayes <- vector("list",9)
for (i in 1:9){
...
fit_summary_bayes$param<-rep(c("sdp","phi", "b1","b0"), times=9 )#add parameter factor
fit_summary_bayes$prop.missing<-rep(prop.miss, times=c(4,4,4,4,4,4,4,4,4)) #add prop of missing data
row.names(fit_summary_bayes)<-1:(9*4) #remove row names
...
ggplot(data=fit_summary_bayes, aes(x=prop.missing, y=mean ))+
  geom_point(aes( color=param, group=param),size=3,position=position_dodge(0.5))+
  theme_classic()+
  geom_errorbar(aes(ymin=min, ymax=high,color=param, group=param), width=0.2, size=0.5,position=position_dodge(0.5))+
  theme(legend.position="top")+
  ...
  theme(axis.text.y=element_text(size=18,colour = "black"))+
  theme(axis.text.x=element_text(size=18,colour = "black"))+
  geom_hline(yintercept = known.data,color=c("black", "darkgray", "green", "blue"))
  
##Calculate denominator of rwci for easier calculation
fit_summary_bayes$rwci.den<-rep(fit_summary_bayes$high[1:4]-fit_summary_bayes$min[1:4], times=9)
fit_summary_bayes$known.param<-rep(c(0.01, 0.8, 0.1, 0.1), times=9)
...
ggplot(data=fit_summary_bayes, aes(x=prop.missing, y=bias ))+
...
saveRDS(fit.stan.miss.amelia, file = "full_sim_week_bayes_sdp_01_phi_8_b0_1_b1_1.RDS")
saveRDS(fit_summary, file = "summary_sim_week_bayes_sdp_01_phi_8_b0_1_b1_1.RDS")
saveRDS(known, file = "known_sim_week_bayes_sdp_01_phi_8_b0_1_b1_1.RDS")
