...
CurSim <- commandArgs(trailingOnly = TRUE) #Look at command line arguements only after the R script
CurSim <- as.numeric(CurSim)
CurSim <- CurSim + 1 # since the Slurm array is 0 indexed

## read in the autocor_01 list ##
  
gauss_sim_randMiss_autoCorr_01 <- readRDS("/project/modelscape/users/mdesierv/gauss_sim_randMiss_autoCorr_01.rds")

# make file for output beforehand in supercomputer folder 
# will put them all together after all run, using the command line
OutFile <- paste("1_results_autocorr_01/", CurSim, "arimavals.csv", sep = "")
